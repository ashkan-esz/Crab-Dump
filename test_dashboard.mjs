...
assert.match(restoresHtml, /class="button primary restore-start"/);
assert.match(restoresHtml, /method="dialog"/);
assert.match(restoresHtml, /method: 'POST'/);
assert.match(restoresHtml, /api\/restores[\s\S]*backup_id/);
assert.match(restoresHtml, /Restore started for/);
...
assert.match(html, /@media \(max-width: 900px\)[\s\S]*grid-template-columns: repeat\(2, minmax\(0, 1fr\)\)/);
assert.match(html, /\.grid \{\s*grid-template-columns: repeat\(3, minmax\(0, 1fr\)\);[\s\S]*@media \(max-width: 900px\)[\s\S]*\.grid \{\s*grid-template-columns: repeat\(2, minmax\(0, 1fr\)\);[\s\S]*@media \(max-width: 640px\)[\s\S]*\.grid \{\s*grid-template-columns: 1fr;/);
assert.doesNotMatch(html, /\.bot-status-card\s*\{[^}]*grid-column:\s*1\s*\/\s*-1/);
assert.match(html, /\.resource-card, \.compression-card \{ grid-column: 1 \/ -1; \}/);
assert.match(html, /\.bot-status-message\.success \{ color: var\(--up\); \}/);
assert.match(html, /\.bot-status-message\.error \{ color: var\(--err\); \}/);
assert.match(html, /class="bot-status-message" role="status" aria-live="polite"/);
assert.match(body, /message\.classList\.remove\('success', 'error'\)/);
assert.match(body, /message\.classList\.add\('success'\)/);
assert.match(body, /message\.classList\.add\('error'\)/);
assert.doesNotMatch(body, /message\.classList\.add\('error'\)[\s\S]*message\.textContent = `Check succeeded/);
...
assert.match(servicesHtml, /id="service-routing-profile"/);
assert.match(servicesHtml, /Use active routing profile for health checks/);
assert.match(servicesBody, /use_active_routing_profile/);
assert.match(servicesBody, /d\?\.use_active_routing_profile \?\? false/);
assert.match(servicesBody, /Health check routing/);
